  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Business CRM</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Growth tracking for {organization?.name}.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Add Customer', path: '/contacts', icon: Plus, color: 'bg-blue-600' },
          { label: 'New Campaign', path: '/campaigns', icon: Send, color: 'bg-green-600' },
          { label: 'Marketing Alerts', path: '/campaigns', icon: MessageSquare, color: 'bg-purple-600' },
          { label: 'Billing Status', path: '/billing', icon: CreditCard, color: 'bg-gray-900' },
        ].map((action) => (
          <Link 
            key={action.label}
            to={action.path}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard 
          ...
          subValue={`${Math.round((campaignStats.delivered / campaignStats.sent) * 100 || 0)}% conversion rate`} 
          icon={Send} 
      ...
      <div className="bg-white dark:bg-[#1e293b] p-6 rounded-2xl border border-gray-100 dark:border-slate-800/50 shadow-sm">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6">Marketing Performance</h3>
