        ...
        <Route path="/campaigns" element={
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Campaigns</h1>
                <p className="text-gray-500 font-bold uppercase text-[8px] tracking-widest mt-0.5">Manage outreach efforts</p>
              </div>
              <button onClick={() => setIsCreatingCampaign(true)} className="px-4 py-2 bg-blue-600 text-white font-black uppercase text-[10px] tracking-widest rounded-lg shadow-lg shadow-blue-600/20 hover:bg-blue-700 transition-all">New Campaign</button>
            </div>
                ...
                <table className="w-full text-left table-auto">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-950/50">
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest">Name</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest">Type</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest">Status</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest">Schedule</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest text-center">Sent</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest text-center text-green-600">Delivered</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest text-center text-red-600">Failed</th>
                      <th className="px-4 py-3 text-[8px] font-black uppercase text-gray-400 tracking-widest text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {campaigns.map(c => (
                      <tr key={c.id || (c as any)._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                        <td className="px-4 py-3">
                          <div className="font-black text-gray-900 dark:text-white text-xs">{c.name}</div>
                          <div className="text-[9px] text-gray-400 uppercase font-bold truncate max-w-[150px]">{c.message}</div>
                        </td>
                        <td className="px-4 py-3 text-[9px] font-black uppercase text-gray-500">{c.type}</td>
                        <td className="px-4 py-3">
                          <span className={cn(
                        ...
                        <td className="px-4 py-3 text-center font-black text-gray-900 dark:text-white text-[10px]">{c.stats?.sent || 0}</td>
                        <td className="px-4 py-3 text-center font-black text-green-600 text-[10px]">{c.stats?.delivered || 0}</td>
                        <td className="px-4 py-3 text-center font-black text-red-600 text-[10px]">{c.stats?.failed || 0}</td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button onClick={() => { setDuplicateData({ ...c, name: `Copy of ${c.name}` }); setIsCreatingCampaign(true); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors text-gray-400 hover:text-blue-600" title="Copy">
                              <Copy className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => { setEditingCampaign(c); setIsCreatingCampaign(true); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors text-gray-400 hover:text-blue-600" title="Edit">
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => handleDeleteCampaign(c.id || (c as any)._id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors text-gray-400 hover:text-red-600" title="Delete">
                    ...
                    {campaigns.length === 0 && (
                      <tr><td colSpan={8} className="px-4 py-16 text-center text-gray-400 font-black uppercase tracking-widest text-[10px]">No active campaigns</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
